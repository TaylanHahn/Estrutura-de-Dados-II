...
int* carregarArquivo(){								
    ...
    //v passa a apontar p o primeiro elemento do vetor
    ...
    //le o arquivo e guarda na posicao i do vetor
    for(int i = 0; i < TAM; i++){
        fscanf(arq,"%d",&v[i]);
    }
    fclose(arq);
...
//vetor, tamanho do vetor, valor procurado, contador de acessos
int buscaSequencial(int v[], int n, int chave, int *acessos){
